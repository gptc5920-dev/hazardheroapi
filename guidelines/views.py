...
class CitizenGuidelineViewSet(viewsets.ReadOnlyModelViewSet):
    queryset=Guideline.objects.filter(status="Published",is_active=True).prefetch_related(Prefetch("translations",queryset=GuidelineTranslation.objects.filter(status="Published").select_related("language"),to_attr="_published_translations"),Prefetch("media",queryset=GuidelineMedia.objects.filter(is_active=True).prefetch_related(Prefetch("translations",queryset=GuidelineMediaTranslation.objects.filter(status="Published").select_related("language"),to_attr="_published_translations")),to_attr="_active_media")); serializer_class=CitizenGuidelineSerializer; permission_classes=[PublicReadOnly]; authentication_classes=[]; lookup_field="slug"; filterset_fields=["category","emergency_type","is_featured"]; search_fields=["title","summary","content"]; ordering_fields=["title","published_at"]
    ...
    def offline_manifest(self,r,slug=None):
        ...
        media=[]
        for item in getattr(guideline,"_active_media",guideline.media.filter(is_active=True)):
            mt,media_language,media_fallback=choose_translation(item,"translations",code); selected=mt.alternative_media_file if mt and mt.alternative_media_file else (item.media_file if item.source_type=="upload" else None); external_url=None if selected else item.external_url; entry={"id":str(item.id),"source_type":"upload" if selected else item.source_type,"type":item.media_type,"provider":"upload" if selected else external_media_provider(external_url),"external_url":external_url,"returned_language":media_language,"used_fallback":media_fallback,"file":file_metadata(r,selected),"subtitle":file_metadata(r,mt.subtitle_file) if mt and mt.subtitle_file else None,"caption_text":mt.caption_text if mt else ""}; media.append(entry); files.extend(x for x in [entry["file"],entry["subtitle"]] if x)
...
class ResponderGuidelineViewSet(ResponderModelViewSet):
    ...
    def publish(self,r,pk=None):
        obj=self.get_object()
        if obj.status=="Published": raise ValidationError({"status":["This guideline is already published."]})
        obj.status="Published"; obj.published_at=timezone.now(); obj.updated_by=r.user; obj.save(); audit(r,"publish",self.module,obj); return Response(self.get_serializer(obj).data)
    @action(detail=True,methods=["post"])
    def archive(self,r,pk=None):
        obj=self.get_object()
        if obj.status=="Archived": raise ValidationError({"status":["This guideline is already archived."]})
        obj.status="Archived"; obj.updated_by=r.user; obj.save(); audit(r,"archive",self.module,obj); return Response(self.get_serializer(obj).data)
