...
class CitizenContactViewSet(viewsets.ReadOnlyModelViewSet):
    queryset=EmergencyContact.objects.filter(is_active=True,is_verified=True); serializer_class=CitizenContactSerializer; permission_classes=[PublicReadOnly]; authentication_classes=[]; filterset_fields=["contact_type","availability","barangay","city_municipality","province"]; search_fields=["organization_name","description","contact_person"]; ordering_fields=["organization_name","priority_order"]
...
class ResponderContactViewSet(ResponderModelViewSet):
    ...
    def verify(self,r,pk=None): obj=self.get_object(); obj.is_verified=True; obj.updated_by=r.user; obj.save(); audit(r,"verify",self.module,obj); return Response(self.get_serializer(obj).data)
